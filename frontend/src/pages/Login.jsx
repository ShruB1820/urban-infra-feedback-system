...
export default function Login() {
  ...
  const [form, setForm] = useState({
    email: "",
    password: "",
    role: "citizen", // "citizen" | "admin" (visual only unless you send it)
    showPw: false,
  });
  ...
  const pwError = useMemo(() => {
    ...
    return form.password.length >= 4 ? "" : "Minimum 6 characters";
  }, [form.password]);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // supports { token, user:{...} } or { id,email,name,role,token }
      const data = res?.data || {};
      ...
      const userObj = data.user || {
        ...
        email: data.email,
        name: data.name,
        role: data.role || (form.role === "admin" ? "admin" : "user"),
      };
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img src={Logo} alt="Brisbane Connect" className="h-10 w-auto" />
          </div>
        </div>
            ...
            <div className="relative overflow-hidden rounded-3xl shadow-xl">
              ...
              <div className="absolute left-0 right-0 bottom-0 p-6 sm:p-8">
                ...
                <p className="mt-2 max-w-2xl text-white/90">
                  A transparent platform connecting citizens with municipal authorities to resolve public infrastructure issues faster.
                </p>
              </div>
            </div>
          </section>
                  ...
                  {loading ? (
                    <span className="inline-flex items-center gap-2">
                      <svg className="h-4 w-4 animate-spin" viewBox="0 0 24 24">
                        <circle cx="12" cy="12" r="10" stroke="white" strokeWidth="4" fill="none" opacity=".35"/>
